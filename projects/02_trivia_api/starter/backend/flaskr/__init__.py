...
def create_app(test_config=None):
    ...
    def add_question():
        ...
        if 'question' not in user_input or 'answer' not in user_input \
                or 'difficulty' not in user_input or 'category' not in user_input:
            abort(404)
        ...
        category_duplicate = Category.query.filter_by(type=str(user_input['category'])).first()
    ...
    def get_question_by_category(category_id):
        ...
        questions_query = Question.query.filter_by(category=str(category_id)).paginate(page, QUESTIONS_PER_PAGE, False)
        questions = list(map(Question.format, questions_query.items))
        if not questions:
            abort(404)
        result = {
    ...
    def get_question_for_quiz():
        if not request.data:
            abort(422)
        search_data = json.loads(request.data.decode('utf-8'))
        if ('quiz_category' not in search_data
                or 'id' not in search_data['quiz_category']
                or 'previous_questions' not in search_data):
            abort(404)
